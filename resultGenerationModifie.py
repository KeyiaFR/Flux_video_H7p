...
countAdjust=0                                                                          #Counter for brightness adjust
flagAdjust=False                                                                       #Flag to stop brightness adjust

#*********Functions**********************************

def DynamicAdjust(img):
    ...
    while(flagAdjust):
        ...
        img.gamma_corr(gamma=1,contrast=1.3,brightness=countAdjust/100)

        ##Ideal image stdev<50 100<median<150
        #https://docs.openmv.io/library/omv.image.html#image.Image.gamma
        #Dont move gamma parametre (adjust non linear)
        #Use constrast parameter static +30% from OG image
        #Move brightness parameter until statics desired
...
def PreProcessing(imgpro):

    #Use this function only in the result of an image difference.

    #imgpro.histeq(True,3)
    #imgpro.bilateral(2, color_sigma=0.1, space_sigma=1)

    #imgpro.gaussian(3,False)
    imgpro.mean(3, threshold=True, offset=5, invert=True) #Dynamic thresholding for binarization
...
blobAreaThreshold = 450                                                                # Minimum area in pixels considered for blob detections
blobAreaDetection = 450                                                                # Minimum area in pixels used for detection Note : Same size as blobAreaThreshold for speed computing
